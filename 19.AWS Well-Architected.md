...
# 3. Crear las recursos y variables GLOBALES del proyecto
...
## 3.4. Crear llave SSH y exportarla a un archivo local (./key.tf)
...
### 4.2.1. Subnet A
- usar el CIDR: var.public.cidr_block_a
- zona: var.public_zone_a
### 4.2.2. Subnet B
- usar el CIDR: var.public.cidr_block_b
...
- instancia t3a.nano
- ami: utilizar el id del ami GLOBAKL creada en el punto 3
```tf
...
# DNS
## Configurar el registro DNS en la zona existente
- Crear la variable global domain = "aws-terralabs.tk"

## La zona ya existe en la cuenta AWS. No es necesario crearla.
...
- Registro tipo A apuntando al balanceador creado anteriormente
```tf
resource "aws_route53_record" "main" {
  ...
  type    = "A"
	...
	alias {
    name                   = var.albWebserver.dns_name
    zone_id                = var.albWebserver.zone_id
